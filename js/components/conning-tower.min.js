    ...
    _createClass(ConningTower, [{
            ...
            $input.keyup(function () {
                ...
                _this.request({
                    ...
                    success: function success(result) {
                        ...
                        如果搜索无结果绘制相应节点
                        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                        var resultMapper = { "xf": "newHouse", "esf": "secondHouse", "rent": "secondHouse" };
                        if (result.data[resultMapper[_this.moduleType] + "Total"] === 0) {
                        ...
                        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                ...
                                result.data[resultMapper[_this.moduleType] + "List"].forEach(function (source) {
        ...
        value: function citySelectorEventDelegate() {
            ...
            $(".city-selector .tabs-frame a").click(function (event) {
                $.cookie(_this2.moduleType + "SelectedCityId", $(event.target).data("cityid"), { "path": "/" });
                $.cookie(_this2.moduleType + "SelectedCityName", $(event.target).text(), { "path": "/" });
                $.cookie(_this2.moduleType + "SelectedCityPinyin", $(event.target).data("pinyin"), { "path": "/" });
                $.cookie(_this2.moduleType + "SelectedCityChina", $(event.target).data("china"), { "path": "/" });
                _this2.cityClick();
                $(".city-selector").hide();
                if (parseInt($(event.target).data("cityid"), 10) !== parseInt($("#visitedCityId").val(), 10)) window.location.href = _this2.combineUrl($(event.target).data("pinyin"));
            });
        }
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        对接口拿到的业务开通城市进行处理
        ...
        value: function location() {
            ...
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                _this3.request({
                    apiUrl: _this3.getCityLolaApiUrl,
