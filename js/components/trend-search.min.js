...
define(function () {
    var that = void 0;
    var houseListType = 'oldhouselist'; // 二手房或者新房来源标识
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    点击icon搜索框的出现
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    var selectFun = function selectFun(select) {
        $('.' + select).click(function () {
            ...
            if (JSON.parse(localStorage.getItem(houseListType))) {
                $('.history').show();
                var searchHistory = JSON.parse(localStorage.getItem(houseListType)).reverse();
    ...
    $('#searchInput').on('keyup input', function () {
        if ($(this).val()) {
            $('.icon-close').show();
            $('.history').hide();
            var listData = void 0; // 搜索联想结果
            var searchaAcWord = ''; // 拼装dome节点
            var sendData = {
                key: $(this).val(),
            ...
            that.request(that.apiUrl.common.acWord, sendData, {
                successCallback: function successCallback(data) {
                    ...
                    if (listData.secondHouseList) {
                        ...
                        var titleName = '';
                        var addreName = '';
                        listData.secondHouseList.forEach(function (item, index) {
                            // 循环出搜索结果
    ...
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    var historyList = function historyList(singleData) {
        var saveLocalStorage = JSON.parse(localStorage.getItem(houseListType)) ? JSON.parse(localStorage.getItem(houseListType)) : [];
        ...
        var houseT = houseListType == "oldhouselist" ? "esf" : "new";
        var cityPinYin = $.cookie('pinyin');
    ...
    return {
        init: function init(c, houseType) {
            that = c;
