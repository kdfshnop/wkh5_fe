...
define(['../components/swiper-3.4.2.jquery.min'], function (Swiper) {
    ...
    return function (container) {
        var swiper = $(container).data('preview-swiper');
        if (!swiper) {
            var $imgs = $(container).find('.preview-image-item'); //获取所有要预览的图片
            var id = new Date().getTime();

            var str = '<div class="swiper-container preview-image" id="' + id + '">' + '<div class="swiper-wrapper">';
            $imgs.each(function (index, img) {
                str += '<div class="swiper-slide">' + '<div class="preview-item">' + '<img src="' + $(img).attr('src') + '">' + '<div class="indicator">' + (index + 1) + '/' + $imgs.length + '</div>' + '</div>' + '</div>';
