...
var ListController = function (_Controller) {
    ...
    function ListController() {
        ...
        var condition = location.href.slice(location.href.lastIndexOf('/') + 1, location.href.length);
        var conditionObject = _this.parseCondition({ condition: condition }); // 转成对象
        _this.choseFun(conditionObject);
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        请求接口 获取城市区域
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        _this.request(_this.apiUrl.rent.list.cityAreas, { houseType: 3, cityId: 3 }, {
        ...
        请求接口 获取地铁
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        _this.request(_this.apiUrl.rent.list.citySubway, { cityId: 3 }, {
            successCallback: function successCallback(data) {
                if (data.status == 1) {
                    var dataRes = data;
    ...
    _createClass(ListController, [{
        ...
        key: 'giveAreasData',
        value: function giveAreasData(dataAreas) {
            console.log(dataAreas.data);
            var dataDic = dataAreas.data;
            var dicAreas = "<li >不限</li>";
            dataDic.forEach(function (item) {
                // 循环渲染地铁线路
                dicAreas += '<li data-id ="' + item.id + '" data-di="di-' + item.id + '">' + item.name + '</li>';
            });
            ...
            $('#dicAreas>li').click(function () {
                // 点击areas更换相应的town
                var dataAreasId = $(this).attr("data-id"); // areas的id 后面循环对比需要
                var dataAreasDi = $(this).attr("data-di"); // 需要传给后台pinURL的参数  areas
                var dataAreasName = $(this).html(); // 获取中文地铁线路名称
                $(this).siblings().removeClass('areas-subway');
                $(this).addClass('areas-subway');
                if (dataAreasName == "不限") {
                    $('#dic>p').html("区域");
        ...
        key: 'giveSubwayData',
        value: function giveSubwayData(dataSubway, url) {
            console.log(dataSubway.data);
            var subwayLine = dataSubway.data;
            var metroLine = '';
            subwayLine.forEach(function (item) {
                // 循环渲染地铁线路
                metroLine += '<li data-id ="' + item.id + '" data-li="li-' + item.id + '">' + item.name + '</li>';
            ...
            $('#metroLine > li').click(function () {
                // 点击地铁线路更换相应的站点
                var dataLineId = $(this).attr("data-id"); // 地铁线路的id 后面循环对比需要
                var dataLineLi = $(this).attr("data-li"); // 需要传给后台pinURL的参数  地铁线路
                var dataLineName = $(this).html(); // 获取中文地铁线路名称
                $(this).siblings().removeClass('areas-subway');
                $(this).addClass('areas-subway');
                subwayLine.forEach(function (item) {
                    if (dataLineId == item.id && item.subList) {
                        var metroStation = "<li>不限</li>";
                        item.subList.forEach(function (item) {
                            // 循环渲染地铁站点
                            metroStation += '<li data-id ="' + item.id + '" data-st="st-' + item.id + '">' + item.name + '</li>';
                        });
        ...
        value: function firstGivePage(conditionObject, self) {
            ...
            if (conditionObject['la']) {
                ...
                var conditionArray = laString.split(',');
