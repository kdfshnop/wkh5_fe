...
var ListController = function (_Controller) {
    ...
    function ListController() {
        ...
        var channelFlag = ''; // 埋点需要的 channel:channel || "",
        if (_this.GetRequest()['channel']) {
            channelFlag = _this.GetRequest()['channel'];
        }
        var conditionObject = _this.parseCondition({ condition: condition }); // 转成对象
        _this.choseFun(conditionObject, url, acWordHouseList);
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        请求接口 获取城市区域
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                ...
                if (data.status == 1 && data.data.length) {
                    ...
                    $('#metroLine > li').click(function () {
                        ...
                        $('#metroStation > li').click(function () {
                            ...
                            if ($(this).html() == "不限") {
                                ...
                                window.location.href = url + _conditionString4 + areasLineSting; // 跳转的URL
                            }
                            $('.bac').hide();
                            $('#dic').children('span').removeClass('direction');
                            $('#dic').find('i').addClass('bacchosed');
                    ...
                    $('#metroStation > li').click(function () {
                        ...
                        if ($(this).html() == "不限") {
                            ...
                            $('#dic > p').html(dataStationName); // 判断赋值给检索title
                            var lineStationString = "li-" + conditionObject['li'] + '-' + dataStationSt; // 合并字符串
                            ...
                            window.location.href = url + _conditionString5 + areasLineSting; // 跳转的URL
                        }
                        $('.bac').hide();
                        $('#dic').children('span').removeClass('direction');
                        $('#dic').find('i').addClass('bacchosed');
        ...
        $('#selfPrConf').click(function (event) {
            ...
            if (lowPr && topPr) {
                ...
                if (lowPr == '' && topPr) {
                    ...
                    window.location = url + _conditionString7 + queryString; // 跳转的URL
                } else if (topPr == '' && lowPr) {
                    ...
                    window.location.href = url + _conditionString8 + queryString; // 跳转的URL
                } else {
                    $('#price>p').html("租金");
                    $('#price').find('i').removeClass('bacchosed');
                    $('#price').removeClass('active-color-top');
                    ...
                    window.location.href = url + _conditionString9 + queryString; // 跳转的URL
                }
                $('.bac').hide();
                $('#price').children('span').removeClass('direction');
                $('.price-total').slideToggle();
        ...
        $('#searchInput').on('keyup input', function (event) {
            if ($(this).val()) {
                ...
                /* 埋点的参数*/
                var bigdata = encodeURIComponent(JSON.stringify({
                    eventName: '1203004',
                    channel: channelFlag || "",
                    type: 2
                }));
                that.request(that.apiUrl.rent.list.acWord, sendData, {
                    successCallback: function successCallback(data) {
                        ...
                        $('#showResult >li').click(function () {
                            ...
                            var saveLocal = saveLocalStorage.reverse();
                ...
                if (acWordHouseList) {
                    if (event.keyCode == 13) {
                        ...
                        var typeS = $('#showResult>li:eq(0)').attr('data-type');
        ...
        搜索初步渲染
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        if (JSON.parse(localStorage.getItem('searchHistory')) && _this.haveNot()) {
            var firtName = JSON.parse(localStorage.getItem('searchHistory')).reverse()[0].key;
        ...
        if (condition) {
            ...
            var newConditionString = conditionString.replace("to", "townId").replace("li", "subwayLine").replace("st", "subwayStation");
            var conditionObj = that.parseCondition({ condition: newConditionString });
            ...
            if (conditionObj['la'] && conditionObj['la'].length == 1) {
                // 判断是对象还是数组
            ...
            if (conditionObj['cp']) {
                // 价格自定义
                var cpArray = conditionObj['cp'].split("townId");
            ...
            if (_this.GetRequest()['districtId']) {
                // 查询？后面参数异步请求
                conditionData['districtId'] = _this.GetRequest()['districtId'];
            } else if (_this.GetRequest()['townId']) {
                conditionData['townId'] = _this.GetRequest()['townId'];
            } else if (_this.GetRequest()['subwayLine']) {
                conditionData['subwayLine'] = _this.GetRequest()['subwayLine'];
            } else if (_this.GetRequest()['subwayStation']) {
                conditionData['subwayStation'] = _this.GetRequest()['subwayStation'];
            } else if (_this.GetRequest()['subEstateId']) {
                conditionData['subEstateId'] = _this.GetRequest()['subEstateId'];
            }
        ...
        _this.pullload(conditionData);

        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        判断是否为今日头条
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        if (_this.GetRequest()['channel'] == "jrttsub") {
        ...
        $('#searchInput').click(function () {
            ...
            if (JSON.parse(localStorage.getItem('searchHistory')) && !acWordHouseList) {
                // Storage取值渲染
                $('body').css('background-color', '#F0F0F0');
                $('.have-result').show();
                ...
                $('#resultHistory >li').click(function () {
                    ...
                    delete conditionObject['li'];delete conditionObject['st'];
                    var conditionString = that.objectToString(conditionObject); // 转换成字符串
                    var valueSearch = $(this).attr('data-value');
                    var typeS = $(this).attr('data-type');
                    window.location.href = url + conditionString + that.checkType(typeS, valueSearch);
                });
            } else {
    ...
    _createClass(ListController, [{
        ...
        key: "choseFun",
        value: function choseFun(conditionObject, url, acWordHouseList) {
            var self = this;
            /* 条件选择点击事件*/
