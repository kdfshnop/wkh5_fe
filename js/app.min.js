...
(function($){
    function Drag(opts) {
        var options = {
            elem:"",
            //callback:function(){} 原来代码中其实已经用不到这个了，因此我注释掉
        };
    ...
    Drag.prototype.bindEvent = function(){
        var _this = this,
            maxMove=0;
        //if(_this.$elem.height()<_this.$elem.parent().height())return;
        
        this.$elem.on('touchstart', function(event) {
            maxMove=_this.$elem.parent().height()-_this.$elem.height();
            if (!_this.locked) {
                _this.startY = event.originalEvent.touches[0].pageY;
            }
        }).on('touchmove', function(event) {
            if (!_this.locked) {
                _this.dragY = _this.endY + event.originalEvent.touches[0].pageY - _this.startY;
                if(_this.dragY>0){
                    _this.dragY=_this.dragY/3;
                }else if(_this.dragY<maxMove){
                    _this.dragY=maxMove+(_this.dragY-maxMove)/3;
                }
                _this.$elem.css("-webkit-transform", "translate(0," + _this.dragY + "px)");
            }
            event.preventDefault();
        }).on('touchend', function(event) {
            var	elemHeight = _this.$elem.height(),
            scrollHeight = _this.$elem.height() - _this.$elem.parent().height();

            _this.endY = _this.dragY;
            var y;
            if(_this.endY==0)return;
            if (_this.endY > 0 || -_this.endY > scrollHeight) {
                if (_this.endY > 0) {
                    y = _this.endY = 0;

                } else if (-_this.endY > scrollHeight) {
                    y = _this.endY = -scrollHeight;
                }
                _this.$elem.on('webkitTransitionEnd transitionend', function() {
                    _this.locked = false;
                    $(this).removeClass('springback-animation');
                    $(this).off('webkitTransitionEnd transitionend');
                })
                .addClass('springback-animation')
                .css("-webkit-transform", "translate(0," + y + "px)");
                setTimeout(function(){
                        _this.locked = false;
                        _this.$elem.removeClass('springback-animation')
 ...
 * @example $.cookie('the_cookie', 'the_value', { expires: 7 * 24 * 60 , path: '/', domain: 'jquery.com', secure: true });
    ...
    $.fn.pullload = function(options) {
        ...
        return this.each(function() {
            ...
            function eventHandler() {                
                ...
                满足条件就触发请求
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                //alert('condition:' + (height + scrollTop) > offset && $(self).attr("data-requestable") == "true");
                var $errorPanel = $('.error-panel');
                if($errorPanel.length == 0){
                    $errorPanel = $('<div class="error-panel" style="overflow: auto; position: fixed; z-index=100; max-height: 100px; width: 100%"></div>').appendTo('body');                    
                }
                $errorPanel.append("condition:" + (height + scrollTop) > offset && $(self).attr("data-requestable") == "true");
...
var Controller = function () {
    function Controller() {
        ...
        if (this.environment === "test") this.staticDomain = "//test01.fe.wkzf";else if (this.environment === "sim") this.staticDomain = "//sim01.fe.wkzf";else if (this.environment === "prod") this.staticDomain = "//cdn01.wkzf.com";
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        工具库路径及应用的控制器路径
         -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    _createClass(Controller, [{
        ...
        key: "request",
        value: function request(apiUrl, data, params) {
            var classSelf = this;
            var type = params.type || "GET";
            var contentType = params.contentType || "application/x-www-form-urlencoded";
            var showLoadingTips = params.showLoadingTips || false;
            var loadingTips = params.loadingTips || "正在加载数据，请稍等...";
            var apiDataType = params.apiDataType || this.apiDataType;
            var errorCallback = params.errorCallback || $.noop;
            var beforeSendCallback = params.beforeSendCallback || $.noop;
            var successCallback = params.successCallback || $.noop;
            var exceptionCallback = params.exceptionCallback || $.noop;
            var isShowErrorTips = true;
            ...
            var options = {
                ...
                success: function success(data) {
                    if (data.status.toString() === "1") successCallback(data);else {
                        $.tips(data.message, 2);
        ...
        key: "addListenerToAssistant",

        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        底部经纪人助手条处理
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        value: function addListenerToAssistant() {
            var _this = this;
            ...
            $(".assistant .wx").click(function () {
                $.modal({
                    ...
                    "buttons": [{ "text": "复制微信号", "className": "copy", "clickCallback": function clickCallback() {
                            $(this).attr("data-clipboard-text", $(".assistant .wx").data("agentwchatid"));
                            var clipboard = new Clipboard("#addWechatModal .copy");
            ...
            $(".assistant .phone").click(function () {
                _this.request(_this.apiUrl.common.dial, { agentId: $("#agentId").val(), houseId: $("#houseId").val(), serviceType: $("#serviceType").val() }, {
                    successCallback: function successCallback(result) {
                        $.modal({
                            ...
                            "buttons": [{ "text": "取消", "clickCallback": function clickCallback() {
                                    $.modal.close("callAgentModal");
                                } }, {
                                "text": "拨打",
                                "className": "phone",
        ...
        key: "addListenerToDownloadApp",
        value: function addListenerToDownloadApp() {
...
var Share = function () {
    ...
    _createClass(Share, [{
        ...
        value: function getApiUrl() {
            ...
            if (environment === "test") apiPrefix = "//10.0.18.79:8134/";else if (environment === "sim") apiPrefix = "//wechat.sim.wkzf/";else apiPrefix = "//wechat.wkzf.com/";
        ...
        key: "wx_onMenuShareTimeline",
        value: function wx_onMenuShareTimeline(title, linkUrl, imgUrl) {
        ...
        key: "wx_onMenuShareAppMessage",
        value: function wx_onMenuShareAppMessage(title, linkUrl, content, imgUrl) {
        ...
        value: function onload() {
            ...
            wx.ready(function () {
                wx.showOptionMenu();
                _this.wx_onMenuShareTimeline(_this.title, _this.linkUrl, _this.imgUrl); //  监听“分享到朋友圈
                _this.wx_onMenuShareAppMessage(_this.title, _this.linkUrl, _this.content, _this.imgUrl); //  监听“发送给朋友”按钮点击
            });
            /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            发送请求
            -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
var Location = function () {
    function Location(_ref) {
        ...
        var businessType = _ref.businessType,
            cityApiUrl = _ref.cityApiUrl,
            identical = _ref.identical,
            investmentFlag = _ref.investmentFlag;
        ...
        window.onpageshow = function () {
            if ($.cookie("citySelectionOpen") && !$.cookie("selectedCityPinyin")) {
                window.location.href = _this.route("shanghai", 43);
                $.cookie(_this.cookieKeyPrefix + "noChose", 1, { path: "/" }); // 用户首次拒绝定位跳到城市列表页但没有选择城市返回之后 设置标识防止再次决绝定位时跳入城市选择页
            }
        };
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    _createClass(Location, [{
        ...
        value: function orientate() {
            ...
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                往cookie里面写经纬度
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                $.cookie(_this2.cookieKeyPrefix + "latitude", latitude, { path: "/" });
                $.cookie(_this2.cookieKeyPrefix + "longitude", longitude, { path: "/" });
                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                根据经纬度反查城市信息
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                $.ajax({
                    url: _this2.cityApiUrl,
                    type: "GET",
                    ...
                    success: function success(result) {
                        ...
                        if (result.status.toString() === "1" && city) {
                            ...
                            写几个cookie的值
                            -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                            $.cookie(_this2.cookieKeyPrefix + "cityName", city.cityName, { path: "/" });
                            $.cookie(_this2.cookieKeyPrefix + "cityPinyin", city.cityPinyin, { path: "/" });
                            $.cookie(_this2.cookieKeyPrefix + "cityId", city.cityId, { path: "/" });
                            $.cookie(_this2.cookieKeyPrefix + "deniedName", "", { path: "/" });
                            /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                            ...
                            if (city.cityId == $.cookie("cityId")) {
                                ...
                                2. 页面异步的话就调数据接口渲染页面
                                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                _this2.identical({ "latitude": latitude, "longitude": longitude });
                            } else {
                                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                如果不同，就组织新的路由跳转
                                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                var newCityId = city.cityId;
                                var newPinyin = city.cityPinyin;
                                city[_this2.businessType + "Business"] ? window.location.href = _this2.route(newPinyin, newCityId) : _this2.investmentFlag({ "investment": true });
                            }
                        } else {
                            if (!($.cookie("cityId") == 43)) {
                                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                写几个cookie的值 为空
                                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                $.cookie(_this2.cookieKeyPrefix + "cityName", "", { path: "/" });
                                $.cookie(_this2.cookieKeyPrefix + "cityPinyin", "", { path: "/" });
                                $.cookie(_this2.cookieKeyPrefix + "cityId", "", { path: "/" });
                                $.cookie(_this2.cookieKeyPrefix + "deniedName", "", { path: "/" });
                                $.tips(result.message, 3);
                                window.location.href = _this2.route("shanghai", 43);
                            }
                        }
                    }
                });
                ...
                switch (error.code) {
                    case error.PERMISSION_DENIED:
                        // 用户阻止了授权
                        $.cookie(_this2.cookieKeyPrefix + "deniedName", "定位服务暂未开启", { path: "/" });
                        _this2.fail();
                        break;
        ...
        value: function timeout() {
            ...
            $.modal({
                ...
                "buttons": [{ "text": "去选择", "className": "goto-select-city", "clickCallback": function clickCallback() {
                        ...
                        window.location.href = "/public/city/select?businessType=" + _this3.businessType + "&channel=" + _this3.GetRequest()['channel']; // 根据channel的值判断来源，用于返回来源埋点需要
                    } }]
            });
        }
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        key: "route",
        value: function route(newPinyin, newCityId) {
            ...
            var moduleName = "esf";
            if (this.businessType === "new") moduleName = "xflist";else if (this.businessType === "rent") moduleName = "rent/?channel=" + this.GetRequest()['channel']; // 根据channel的值判断来源，用于返回来源埋点需要
...
var ParamGenerator = function () {
    function ParamGenerator(funcs) {
        _classCallCheck(this, ParamGenerator);

        this.funcs = $.extend({}, ParamGenerator.DEFAULT, funcs);
    }

    _createClass(ParamGenerator, [{
        key: 'generateParamObj',
        value: function generateParamObj(obj, initObj) {
...
ParamGenerator.convert2FilterParam = function (obj) {
    ...
    }, _defineProperty(_funcs, 'lon', ParamGenerator.createMappingFunc(1, "longitude")), _defineProperty(_funcs, 'lat', ParamGenerator.createMappingFunc(1, "latitude")), _funcs);
