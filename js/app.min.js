    ...
    _createClass(Location, [{
        ...
        value: function orientate() {
            ...
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                往cookie里面写经纬度
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                $.cookie(_this.cookieKeyPrefix + "latitude", latitude, { path: "/" });
                $.cookie(_this.cookieKeyPrefix + "longitude", longitude, { path: "/" });
                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                根据经纬度反查城市信息
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                $.ajax({
                    url: _this.cityApiUrl,
                    type: "GET",
                    ...
                    success: function success(result) {
                        ...
                        if (result.status.toString() === "1") {
                            ...
                            写几个cookie的值
                            -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                            $.cookie(_this.cookieKeyPrefix + "cityName", city.cityName, { path: "/" });
                            /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                            ...
                            if (city.cityId == $.cookie("cityId")) {
                                ...
                                2. 页面异步的话就调数据接口渲染页面
                                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                _this.identical({ "latitude": latitude, "longitude": longitude });
                            } else {
                                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                如果不同，就组织新的路由跳转
                                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                                var newCityId = city[_this.businessType + "Business"] ? city.cityId : 43;
                                var newPinyin = city[_this.businessType + "Business"] ? city.cityPinyin : "shanghai";
                                window.location.href = _this.route(newPinyin, newCityId);
                            }
                        } else {
                            $.tips(data.message, 3);
            ...
            if ($.cookie("citySelectionOpen") && !$.cookie("userSelectedCity")) {
                ...
                $.modal({
                    ...
                    "buttons": [{ "text": "去选择", "className": "goto-select-city", "clickCallback": function clickCallback() {
                            ...
                            window.location.href = "/common/city/select?businessType=" + _this2.businessType;
                        } }]
                });
            }
        }
