...
class Location {
    constructor({ businessType , cityApiUrl , identical ,investmentFlag}) {
        ...
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        onPageShow的时候去检查用户点击了城市选择页面但是没有选择任何城市  selectedCityPinyin
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        window.onpageshow = () => {
            if( $.cookie("citySelectionOpen") && ! $.cookie("selectedCityPinyin")) {
        ...
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        根据$.cookie("selectedCityPinyin")的值是否有来决定是否需要地理定位 ;
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        else {
            if( ! $.cookie("selectedCityPinyin")) this.orientate() ;
        }
    }
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        navigator.geolocation.getCurrentPosition( (position) => {
            ...
            $.ajax({
                ...
                success : (result) => {
                    ...
                    if (result.status.toString() === "1" && city) {
                        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                        写几个cookie的值
                        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/                        
    ...
    route( newPinyin , newCityId ) {
        ...
        if( this.businessType === "esfTrend" ) return "/"+newPinyin+"/trend/esf";
        if( this.businessType === "newTrend" ) return "/"+newPinyin+"/trend/new";
        let moduleName = "esf" ;
