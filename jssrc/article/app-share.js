...
class ArticleShareController extends Controller {
    ...
    refreshComment() {
        ...
        var PN = new Pagination({
            ...
            callback: function(pageIndex) {
                ...
                // 调用接口获取评论列表
                // TODO::
                classSelf.Operation.getCommentList({
                    articleId: classSelf.articleId,
                    pageIndex: itemIndex
                }, function(result) {
    ...
    bindEvent() {
            ...
            if ($icon.hasClass('icon-appraise')) {
                ...
                classSelf.Operation.praise({
                    ...
                    if(data.status ===1 && data.data){
                        ...
                        _.html('<i class="wk_iconfont icon-zan"></i><span>' + data.data+'</span>');
                    }else{

                    }
                });
            }
            if ($icon.hasClass('icon-zan')) {
                classSelf.Operation.showMessage('您已经赞过');
            }
        });
        ...
        $('.comment').on('click', '.submit', function(event) {
            ...
            if (comment.length === 0) {
                classSelf.Operation.showMessage('评论内容不能为空');
                return false;
            }     
            ...
            if(data.status===1 && data.data){
                // TODO:提示品论成功
                //classSelf.Operation.showMessage('评论成功');
                ...
                classSelf.Operation.showMessage('评论失败');
            }
            },function(){
                $('.comment .submit').css({
                    'color': '#4081D6',
