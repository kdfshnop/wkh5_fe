...
define([],function(){
    ...
    function bindEvent() {
        ...
        // 关闭筛选弹层
        function hide(){            
            var event = $.Event('hide.filter',{relatedTarget: self});
            ...
            event = $.Event('hidden.filter', {relatedTarget: self});
            self.$el.trigger(event);
        }

        // 打开筛选弹层
        function show(index){
            var event = $.Event('show.filter',{relatedTarget: self});
            ...
            event = $.Event('shown.filter', { relatedTarget: self});
            self.$el.trigger(event);
        }
    ...
    // 设置弹层的高度
    function setMaxHeight(nondynamic){
        //var winHeight = $(window).height() - 80;
        var winHeight = !!nondynamic? ($(window).height() - 87) : ( $(window).height() - $('.filter').position().top);
